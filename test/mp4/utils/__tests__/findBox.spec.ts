import { findBox } from '../../../../src/mp4/utils/findBox';
...
describe('findBox', () => {
  ...
  it('find videoStco box', () => {
    expect(findBox(mp4BoxTree, 'videoStco').samples[0]).toBe(9271);
  });

  it('find audioStco box', () => {
    expect(findBox(mp4BoxTree, 'audioStco').samples[0]).toBe(16729);
  });

  it('find videoStts box', () => {
    expect(findBox(mp4BoxTree, 'videoStts').samples[0].sample_count).toBe(251);
  });

  it('find audioStts box', () => {
    expect(findBox(mp4BoxTree, 'audioStts').samples[0].sample_count).toBe(430);
  });
  ...
  it('find videoStss box', () => {
    expect(findBox(mp4BoxTree, 'videoStss').sampleNumbers).toHaveLength(2);
  });

  it('find videoStsz box', () => {
    expect(findBox(mp4BoxTree, 'videoStsz').sampleSizes[0]).toBe(7458);
  });

  it('find audioStsz box', () => {
    expect(findBox(mp4BoxTree, 'audioStsz').sampleSizes[0]).toBe(371);
  });
